...
const MiniPage = () => {
  ...
  useEffect(() => {
    ...
    // 延迟确保内容渲染完毕后再计算窗口大小
    const initTimer = setTimeout(updateWindowSize, 100);

    const window = getCurrentWebviewWindow();

    // 监听 Rust 侧 show() 后发出的 resize 事件
    const unlistenShown = window.listen("mini-window-shown", () => {
      // 等两帧确保布局完成再计算
  ...
  useEffect(() => {
    ...
    listen("verge://refresh-verge-config", () => {
      revalidateQueries([["getVergeConfig"]]);
    }).then((fn) => {
      unlisten = fn;
    });
        ...
        >
          <MenuList dense sx={{ py: 0.5 }}>
            <MenuItem onClick={toggleAlwaysOnTop} sx={{ minWidth: 140 }}>
              <ListItemText
                primary={t("home.components.mini.alwaysOnTop" as any)}
              />
              {isAlwaysOnTop && (
            ...
            <MenuItem onClick={toggleAutoHide} sx={{ minWidth: 140 }}>
              <ListItemText
                primary={t("home.components.mini.autoHideFullscreen" as any)}
              />
            ...
            >
              <ListItemText
                primary={t("home.components.mini.closeMiniWindow" as any)}
              />
            </MenuItem>
          </MenuList>
        </Paper>
